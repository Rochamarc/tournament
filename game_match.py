...
# This is all became a class Game: with staticmethod methods
#
#
...
class Game:
    ...
    def game_match(match_round, competition, home_team, away_team, verbose=False):
        ...
        goals = Game.match_actions(home_team, away_team, 45) # start match 
        ...
        home_team.register_game(home_goals, away_goals, 'group_stage')
        away_team.register_game(away_goals, home_goals, 'group_stage')
    ...
    def match_actions(home_team, away_team, match_time):
        ...
        for i in range(match_time):
            if i % 2 == 0:
                ...
                # defensives
                keeper = Game.select_player(away_players, 'goalkeeper')
                defensor = Game.select_player(away_players, 'defender')

                # attackers
                midfielder = Game.select_player(home_players, 'midfielder')
                attacker = Game.select_player(home_players, 'attacker')

                """ Ataque = toca + chuta """
                """ Defesa = corta + defende """

                touch = Game.decision(midfielder.overall)
                tackle = Game.decision(defensor.overall)
                ...
                if home_subs > 0:
                    ...
                    if sub:
                        confirm_sub = Game.subs(home_players, home_bench)
                ...
                if tackle:
                    ...
                    if touch:
                        ...
                        finish = Game.decision(attacker.overall)
                        defense = Game.decision(keeper.overall)
                ...
                # defensives
                keeper = Game.select_player(home_players, 'goalkeeper')
                defensor = Game.select_player(home_players, 'defender')

                # attackers
                midfielder = Game.select_player(away_players, 'midfielder')
                attacker = Game.select_player(away_players, 'attacker')
                ...
                if away_subs > 0:
                    ...
                    if sub:
                        confirm_sub = Game.subs(away_players, away_bench)
        ...
        Game.check_game_stats(home_players, home_goal, h_player_goals, away_players, away_goal, a_player_goals)
